...
if ( ! class_exists( 'WooCommerceOrderFailedNotification' ) ) {
	...
	class WooCommerceOrderFailedNotification extends SMSNotification {
		...
		/**
		 * This function is used to get the instance of the WooCommerceOrderFailedNotification class.
		 *
		 * @param array $config Configuration array.
		 * @return WooCommerceOrderFailedNotification Object containing the instance of the class.
		 */
		public static function mo_otp_get_instance( $config = null ) {
			if ( null === self::$instance ) {
				...
				$default_sms_message = MoWcAddOnMessages::showMessage(
					MoWcAddOnMessages::ORDER_FAILED_SMS
				);

				// Default configuration.
				$default_config = array(
					'title'             => 'Order Failed',
					'page'              => 'wc_order_failed_notif',
					'is_enabled'        => false,
					'tool_tip_header'   => 'ORDER_FAILED_NOTIF_HEADER',
					'tool_tip_body'     => 'ORDER_FAILED_NOTIF_BODY',
					'recipient'         => 'customer',
					...
					'page_header'       => __( 'ORDER FAILED NOTIFICATION SETTINGS', 'miniorange-otp-verification' ),
					'page_description'  => __( 'SMS notifications settings for Order failure SMS sent to the users', 'miniorange-otp-verification' ),
		...
		public function send_sms( array $args ) {
			...
			if ( MoUtility::mo_is_whatsapp_notif_enabled() ) {
				MoUtility::mo_send_whatsapp_notif( $phone_number, $this->template_name, $sms_tags, 'ORDER_FAILED' );
			} else {
				MoUtility::send_phone_notif( $phone_number, $sms_body, 'ORDER_FAILED' );
			}
		}
	}
}
