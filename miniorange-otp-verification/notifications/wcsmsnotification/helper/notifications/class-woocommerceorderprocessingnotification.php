<?php
/**
 * Helper functions for Woocommerce Order Processing Notifications
 *
...
if ( ! class_exists( 'WooCommerceOrderProcessingNotification' ) ) {
	...
	class WooCommerceOrderProcessingNotification extends SMSNotification {
		...
		/**
		 * This function is used to get the instance of the WooCommerceOrderProcessingNotification class.
		 *
		 * @param array $config Configuration array.
		 * @return WooCommerceOrderProcessingNotification Object containing the instance of the class.
		 */
		public static function mo_otp_get_instance( $config = null ) {
			if ( null === self::$instance ) {
				...
				$default_sms_message = MoWcAddOnMessages::showMessage(
					MoWcAddOnMessages::PROCESSING_ORDER_SMS
				);

				// Default configuration.
				$default_config = array(
					'title'             => 'Processing Order',
					'page'              => 'wc_order_processing_notif',
					'is_enabled'        => false,
					'tool_tip_header'   => 'ORDER_PROCESSING_NOTIF_HEADER',
					'tool_tip_body'     => 'ORDER_PROCESSING_NOTIF_BODY',
					'recipient'         => 'customer',
					...
					'page_header'       => __( 'ORDER PROCESSING NOTIFICATION SETTINGS', 'miniorange-otp-verification' ),
					'page_description'  => __( 'SMS notifications settings for Order Processing SMS sent to the users', 'miniorange-otp-verification' ),
		...
		public function send_sms( array $args ) {
			...
			if ( MoUtility::mo_is_whatsapp_notif_enabled() ) {
				MoUtility::mo_send_whatsapp_notif( $phone_number, $this->template_name, $sms_tags, 'ORDER_PROCESSING' );
			} else {
				MoUtility::send_phone_notif( $phone_number, $sms_body, 'ORDER_PROCESSING' );
			}
		}
	}
}
