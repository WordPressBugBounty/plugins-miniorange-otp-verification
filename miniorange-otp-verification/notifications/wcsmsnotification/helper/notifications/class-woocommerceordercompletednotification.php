...
if ( ! class_exists( 'WooCommerceOrderCompletedNotification' ) ) {
	...
	class WooCommerceOrderCompletedNotification extends SMSNotification {
		...
		/**
		 * This function is used to get the instance of the WooCommerceOrderCompletedNotification class.
		 *
		 * @param array $config Configuration array.
		 * @return WooCommerceOrderCompletedNotification Object containing the instance of the class.
		 */
		public static function mo_otp_get_instance( $config = null ) {
			...
			if ( null === self::$instance ) {
				...
				// Default SMS content for order completion.
				$default_sms_message = MoWcAddOnMessages::showMessage(
					MoWcAddOnMessages::ORDER_COMPLETED_SMS
				);

				// Default configuration array.
				$default_config = array(
					'title'             => 'Order Completed',
					'page'              => 'wc_order_completed_notif',
					'is_enabled'        => false,

					'tool_tip_header'   => 'ORDER_CANCELLED_NOTIF_HEADER', // As in original.
					'tool_tip_body'     => 'ORDER_CANCELLED_NOTIF_BODY',   // As in original.
					...
					'page_header'       => __( 'ORDER COMPLETED NOTIFICATION SETTINGS', 'miniorange-otp-verification' ),
					'page_description'  => __( 'SMS notifications settings for Order Completion SMS sent to the users', 'miniorange-otp-verification' ),
				...
				// Assign config values to class instance.
		...
		public function send_sms( array $args ) {
			...
			if ( MoUtility::mo_is_whatsapp_notif_enabled() ) {
				MoUtility::mo_send_whatsapp_notif( $phone_number, $this->template_name, $sms_tags, 'ORDER_COMPLETED' );
			} else {
				MoUtility::send_phone_notif( $phone_number, $sms_body, 'ORDER_COMPLETED' );
			}
		}
	}
}
