...
if ( ! class_exists( 'WooCommerceOrderOnHoldNotification' ) ) {
	...
	class WooCommerceOrderOnHoldNotification extends SMSNotification {
		...
		/**
		 * This function is used to get the instance of the WooCommerceOrderOnHoldNotification class.
		 *
		 * @param array $config Configuration array.
		 * @return WooCommerceOrderOnHoldNotification Object containing the instance of the class.
		 */
		public static function mo_otp_get_instance( $config = null ) {
			if ( null === self::$instance ) {
				...
				$default_sms_message = MoWcAddOnMessages::showMessage(
					MoWcAddOnMessages::ORDER_ON_HOLD_SMS
				);

				// Default configuration.
				$default_config = array(
					'title'             => 'Order on-hold',
					'page'              => 'wc_order_on_hold_notif',
					'is_enabled'        => false,
					'tool_tip_header'   => 'ORDER_ON_HOLD_NOTIF_HEADER',
					'tool_tip_body'     => 'ORDER_ON_HOLD_NOTIF_BODY',
					'recipient'         => 'customer',
					...
					'page_header'       => __( 'ORDER ON-HOLD NOTIFICATION SETTINGS', 'miniorange-otp-verification' ),
					'page_description'  => __( 'SMS notifications settings for Order on-hold SMS sent to the users', 'miniorange-otp-verification' ),
				...
				// Apply config values to class properties.
		...
		public function send_sms( array $args ) {
			...
			if ( MoUtility::mo_is_whatsapp_notif_enabled() ) {
				MoUtility::mo_send_whatsapp_notif( $phone_number, $this->template_name, $sms_tags, 'ORDER_ON_HOLD' );
			} else {
				MoUtility::send_phone_notif( $phone_number, $sms_body, 'ORDER_ON_HOLD' );
			}
		}
	}
}
